...
{
	this.smokeBurst = Global.game.add.emitter( 0,0, 256 );
...
{
	var trail = Global.game.add.emitter( x, y - 4, 8 );
...
{
	var trail = Global.game.add.emitter( x, y - 4, 32 );
	//this.smokeTrail.blendMode = Phaser.blendModes.ADD;
	trail.particleClass = this.SmokeTrailParticle;
	trail.setAlpha( 0.8, 0.0, 4000 )
	//trail.setScale( 1.0, 1, 1.0, 1, 6000, Phaser.Easing.Quintic.Out );
...
{
	this.rubbleBurst = Global.game.add.emitter( 0, 0, 240 );
...
{
	this.floorRubbleBurst = Global.game.add.emitter( 0, 0, 128 );
...
{
	var flame = Global.game.add.emitter( x, y, 1 );
